...
test.describe("Complete Excel Import Flow", () => {
  ...
  test("should complete full import workflow with Italian headers", async ({
    page,
  }) => {
    ...
    const testFile = path.join(
      __dirname,
      "test-data",
      "italian-headers-sample.xlsx",
    );
    await page.setInputFiles('[data-testid="input-excel-file"]', testFile);

    // Verify file upload success
    await expect(
      page.locator('[data-testid="text-file-uploaded"]'),
    ).toContainText("italian-headers-sample.xlsx");

    // Check preview data
    await page.click('[data-testid="button-preview-data"]');
    await expect(
      page.locator('[data-testid="text-preview-stats"]'),
    ).toContainText("clients found");
    ...
    await expect(
      page.locator('[data-testid="text-sync-status"]'),
    ...
    // Monitor progress
    await expect(
      page.locator('[data-testid="text-progress-message"]'),
    ).toContainText("Processing row", { timeout: 60000 });

    // Wait for completion
    await expect(
      page.locator('[data-testid="text-sync-final-status"]'),
    ).toContainText("Sync completed", { timeout: 120000 });

    // Verify results
    const resultsText = await page
      .locator('[data-testid="text-sync-results"]')
      .textContent();
  ...
  test("should handle invalid Excel files gracefully", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="button-proceed-sync"]'),
    ).not.toBeVisible();
  });

  // TEST-E2E-003: Progress Tracking - Large Dataset
  test("should track progress correctly for large datasets", async ({
    page,
  }) => {
    await page.goto("/data/import");
...
test.describe("Duplicate Detection User Experience", () => {
  ...
  test("should display duplicate warnings to user", async ({ page }) => {
    ...
    const duplicatesFile = path.join(
      __dirname,
      "test-data",
      "contains-duplicates.xlsx",
    );
    ...
    await expect(
      page.locator('[data-testid="text-duplicates-found"]'),
    ).toContainText("duplicates detected", { timeout: 60000 });
    ...
    await expect(
      page.locator('[data-testid="modal-duplicates"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="list-duplicate-entries"]'),
    ).toContainText("±");
  });
});
...
test.describe("Data Validation User Feedback", () => {
  ...
  test("should provide column mapping feedback", async ({ page }) => {
    ...
    const unclearHeaders = path.join(
      __dirname,
      "test-data",
      "unclear-headers.xlsx",
    );
    await page.setInputFiles(
      '[data-testid="input-excel-file"]',
      unclearHeaders,
    ...
    await expect(
      page.locator('[data-testid="text-column-mapping-score"]'),
    ).toContainText("confidence");

    // Check if manual mapping interface appears
    if (
      await page.locator('[data-testid="section-manual-mapping"]').isVisible()
    ) {
      // Test manual column mapping
      await page.selectOption(
        '[data-testid="select-column-mapping-0"]',
        "assistedPersonFirstName",
      );
      await page.selectOption(
        '[data-testid="select-column-mapping-1"]',
        "assistedPersonLastName",
      );

      // Verify mapping updates
      await expect(
        page.locator('[data-testid="text-mapping-status"]'),
  ...
  test("should show date format validation results", async ({ page }) => {
    ...
    const problematicDates = path.join(
      __dirname,
      "test-data",
      "problematic-dates.xlsx",
    );
    await page.setInputFiles(
      '[data-testid="input-excel-file"]',
      problematicDates,
    ...
    await expect(
      page.locator('[data-testid="section-date-warnings"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="text-invalid-dates-count"]'),
    ).toContainText("invalid dates");

    // View detailed date issues
    await page.click('[data-testid="button-view-date-issues"]');
    await expect(
      page.locator('[data-testid="list-date-issues"]'),
    ).toContainText("32/13/2025");
  });
});
...
test.describe("Mobile Responsiveness", () => {
  ...
  test("should work correctly on mobile devices", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="button-preview-data"]'),
    ).toBeVisible();
    ...
    await expect(
      page.locator('[data-testid="section-preview-results"]'),
    ).toBeVisible();
...
test.describe("Accessibility Compliance", () => {
  ...
  test("should be accessible to screen readers", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="input-excel-file"]'),
  ...
  test("should work in high contrast mode", async ({ page }) => {
    ...
    const buttonStyles = await page
      .locator('[data-testid="button-preview-data"]')
      .evaluate((el) => {
...
test.describe("Performance Testing", () => {
  ...
  test("should load import page within performance budget", async ({
    page,
  }) => {
...
export { createTestExcelFile, setupE2ETestData, cleanupE2ETestData };
