  ...
  useEffect(() => {
    ...
    if (budgetData && budgetData.length > 0 && selectedAllocations.size === 0) {
      ...
      budgetData.forEach((budget: any) => {
        ...
        const isDirectAssistance =
          budget.budgetTypeName?.toLowerCase().includes("assistenza diretta") ||
          budget.budgetTypeName?.toLowerCase().includes("direct assistance");

        console.log(
          "Is Direct Assistance?",
          isDirectAssistance,
          "Budget Type:",
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh]">
        <DialogHeader>
          ...
          <DialogDescription>
            Review and allocate {staffName}'s compensation for{" "}
            {format(new Date(periodStart), "MMM d")} -{" "}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <Card className="p-4">
              ...
              <div className="text-2xl font-bold">
                {budgetData
                  ? new Set(budgetData.map((b) => b.clientId)).size
                  : 0}
              </div>
            </Card>
          </div>
              ...
              <Table>
                ...
                <TableBody>
                  {(() => {
                    ...
                    const groupedByClient = budgetData.reduce(
                      (acc, budget) => {
                    ...
                    return Object.values(groupedByClient)
                      ...
                      .map((clientGroup) => {
                        ...
                        const serviceTypeGroups = clientGroup.entries.reduce(
                          (acc, entry) => {
                        ...
                        let rowIndex = 0;
                        const totalRows =
                          Object.values(serviceTypeGroups).length; // One row per service type now

                        return Object.values(serviceTypeGroups).map(
                          (serviceGroup: any) => {
                            ...
                            const availableBudgets =
                              serviceGroup.budgetOptions.filter(
                                (b: any) => b.available > 0,
                              );

                            // Find selected allocation for this service
                            const selectedBudgetKey = Array.from(
                              selectedAllocations.keys(),
                            ).find((key) =>
                              key.startsWith(`${serviceGroup.serviceType}-`),
                            );
                            const selectedBudget = selectedBudgetKey
                              ? serviceGroup.budgetOptions.find(
                                  (b: any) =>
                                    selectedBudgetKey ===
                                    `${serviceGroup.serviceType}-${b.allocationId}`,
                                )
                              : null;

                            return (
                              <TableRow
                                key={`${serviceGroup.serviceType}-${clientGroup.clientId}`}
                              >
                                {isFirstRow && (
                                  <TableCell
                                    rowSpan={totalRows}
                                ...
                                <TableCell>
                                  ...
                                  <div className="text-xs text-muted-foreground">
                                    {serviceGroup.totalHours.toFixed(1)}h
                                  </div>
                                </TableCell>
                                <TableCell>
                                  <div className="font-medium">
                                ...
                                <TableCell>
                                  {(() => {
                                    ...
                                    const isDirectAssistance =
                                      serviceGroup.budgetOptions.some(
                                        (b: any) =>
                                          b.budgetTypeName
                                            ?.toLowerCase()
                                            .includes("assistenza diretta") ||
                                          b.budgetTypeName
                                            ?.toLowerCase()
                                            .includes("direct assistance"),
                                      );
                                    ...
                                    if (isDirectAssistance) {
                                      ...
                                      if (
                                        !selectedAllocations.has(fallbackKey)
                                      ) {
                                        setTimeout(() => {
                                          const newAllocations = new Map(
                                            selectedAllocations,
                                          ...
                                          newAllocations.set(fallbackKey, {
                                            clientBudgetAllocationId:
                                              "direct-assistance-fallback",
                                            clientId:
                                              "direct-assistance-client",
                                            budgetTypeId:
                                              "type-direct-assistance",
                                            timeLogIds:
                                              serviceGroup.timeLogs.map(
                                                (log: any) => log.id,
                                              ),
                                            allocatedAmount:
                                              serviceGroup.totalCost,
                                            allocatedHours:
                                              serviceGroup.totalHours,
                                          });
                                    ...
                                    return availableBudgets.length > 0 ? (
                                      ...
                                      >
                                        ...
                                        <SelectContent>
                                          ...
                                          {availableBudgets.map(
                                            (budget: any) => {
                                              const isSelected =
                                                selectedBudgetKey ===
                                                `${serviceGroup.serviceType}-${budget.allocationId}`;
          ...
          {remainingToAllocate > 0 && (
            <Alert>
              ...
              <AlertDescription>
                €{remainingToAllocate.toFixed(2)} of the compensation has not
                been allocated to any budget. This amount will need to be
                handled separately.
              </AlertDescription>
            </Alert>
          )}
