...
export default function CompensationBudgetAllocationPage() {
  ...
  const { data: compensation, isLoading: compensationLoading } = useQuery<Compensation>({
    queryKey: [`/api/compensations/${compensationId}`],
    enabled: !!compensationId,
  });

  // Fetch budget availability for this compensation period
  const { data: budgetData, isLoading: budgetLoading } = useQuery<BudgetAvailability[]>({
  ...
  const approveWithBudgetMutation = useMutation({
    mutationFn: async (allocations: Array<{
      ...
      return apiRequest(`/api/compensations/${compensationId}/approve-with-budget`, {
        method: 'POST',
        body: JSON.stringify({ budgetAllocations: allocations }),
      });
    },
    onSuccess: () => {
  ...
  if (!compensation || !budgetData) {
    return (
      <div className="flex flex-col items-center justify-center h-screen">
        ...
        <p className="text-lg text-muted-foreground">Compensation record not found</p>
        <Button 
          variant="outline" 
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Allocate Compensation to Client Budgets</h1>
          <p className="text-muted-foreground mt-2">
            Review and allocate {compensation.staffName}'s compensation for {format(new Date(compensation.periodStart), 'MMM d')} - {format(new Date(compensation.periodEnd), 'MMM d, yyyy')}
          </p>
        </div>
        <Button 
          variant="outline" 
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-green-600">Allocated</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-yellow-600">Remaining</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {(() => {
                    ...
                    return Array.from(serviceGroups.values()).map((serviceGroup, idx) => {
                      // Get available budgets for this service (budgets with available > 0)
                      const availableBudgets = serviceGroup.budgets.filter(b => b.available > 0);
                      ...
                      const allocatedAmount = selectedBudget 
                        ? selectedAllocations.get(selectedBudget.allocationId)?.allocatedAmount || 0
                        : 0;
                      ...
                      return (
                        <TableRow key={idx}>
                          <TableCell>
                            <div className="font-medium">{serviceGroup.clientName}</div>
                            <div className="text-sm text-muted-foreground">ID: {serviceGroup.clientId}</div>
                          </TableCell>
                          <TableCell>
                            <div className="flex flex-col gap-1">
                              <span className="font-medium">{serviceGroup.serviceType}</span>
                          ...
                          <TableCell>
                            <Select
                              value={selectedBudget?.allocationId || ""}
                              onValueChange={(value) => {
                                // Clear any previous selection for this service
                                serviceGroup.budgets.forEach(b => {
                                  selectedAllocations.delete(b.allocationId);
                                });

                                if (value) {
                                  const budget = serviceGroup.budgets.find(b => b.allocationId === value);
                                  if (budget) {
                                  ...
                                  setSelectedAllocations(new Map(selectedAllocations));
                                }
                              }}
                            >
                              ...
                              <SelectContent>
                                ...
                                {serviceGroup.budgets
                                  ...
                                  .map(budget => (
                                    <SelectItem key={budget.allocationId} value={budget.allocationId}>
