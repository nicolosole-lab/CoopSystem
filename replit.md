...
- **PROCEDURA VERIFICA COMPENSI COMPLETATA (18/08/2025)**: Creata procedura completa di verifica sistema compensi con test automatizzati. Inclusi script `scripts/verify-compensation-calculations.js` per test automatico, guida manuale `scripts/manual-verification-steps.md`, e documentazione `PROCEDURA_VERIFICA_COMPENSI.md`. Test verificano trigger PostgreSQL, calcoli automatici, editing inline, export PDF/CSV e consistenza dati. Identificati issues tipo dati (alcune colonne character varying vs numeric).
