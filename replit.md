...
- **Inline Editing System 95% RESOLVED**: Sistema editing inline per tabella compensi funziona correttamente per tariffe staff (confermato dall'utente) e per ore feriali/festive (confermato dai log). Risolto conflitto endpoint duplicato, mapping camelCase/snake_case, e validazione campi numerici. Implementato endpoint temporaneo /api/inline-compensation/:id con field mapping: regularHours→regular_hours, holidayHours→holiday_hours, totalMileage→total_mileage. Ultimo blocco tecnico su endpoint HTTP 400 da risolvere per campo chilometri.
